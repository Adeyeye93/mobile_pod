...
class RtmpAudioPublisher(
    ...
    companion object {
        private const val TAG = "RtmpAudioPublisher"
        private const val SAMPLES_PER_FRAME = 1024
        private const val WAVEFORM_POINTS = 64 // points sent to JS per frame
    }
    ...
    private fun initAudio() {
        val bufferSize =
                AudioRecord.getMinBufferSize(
                        sampleRate,
                        if (channels == 2) AudioFormat.CHANNEL_IN_STEREO
    ...
    private fun captureLoop() {
        val pcmBuffer = ByteArray(SAMPLES_PER_FRAME * channels * 2)
        val silenceBuffer = ByteArray(SAMPLES_PER_FRAME * channels * 2) // for mute
        val info = MediaCodec.BufferInfo()
        ...
        while (running) {
            ...
            // Downsample PCM to WAVEFORM_POINTS normalised float values [-1, 1]
            val waveform = FloatArray(WAVEFORM_POINTS)
            val step = read / (WAVEFORM_POINTS * 2) // 2 bytes per sample
            for (i in 0 until WAVEFORM_POINTS) {
                val byteIndex = i * step * 2
                if (byteIndex + 1 < read) {
                    val lo = pcmBuffer[byteIndex].toInt() and 0xFF
                    val hi = pcmBuffer[byteIndex + 1].toInt()
                    val sample = (hi shl 8) or lo
                    waveform[i] = sample / 32768f // normalise to [-1, 1]
                }
            }
            onWaveform?.invoke(waveform)
            // ──────────────────────────────────────────────────────────────

            // If muted send silence instead of real audio
            val encoderInput = if (muted) silenceBuffer else pcmBuffer

            fileOutputStream?.write(encoderInput, 0, read)

            val inIndex = codec!!.dequeueInputBuffer(10_000)
            if (inIndex >= 0) {
                codec!!.getInputBuffer(inIndex)?.apply {
                    clear()
                    put(encoderInput, 0, read)
                }
                codec!!.queueInputBuffer(inIndex, 0, read, timestampMs * 1000L, 0)
                timestampMs += read * 1000 / (sampleRate * channels * 2)
            }
